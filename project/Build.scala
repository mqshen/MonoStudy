...
object Build extends sbt.Build {
  ...
  lazy val root = Project("mono", file("."))
    .aggregate(kesque, mono_base, mono_eth)
    .settings(basicSettings: _*)
    .settings(Formatting.buildFileSettings: _*)
    .settings(noPublishing: _*)
    .settings(net.virtualvoid.sbt.graph.DependencyGraphSettings.graphSettings: _*)
    .settings(libraryDependencies ++= Dependencies.basic ++ Dependencies.akka)
    .settings(Packaging.settings)
    .settings(
      mainClass in Compile := Some("mono.mono")
    )

  lazy val mono_base = Project("mono-base", file("mono-base"))
    .settings(basicSettings: _*)
    .settings(noPublishing: _*)
    .settings(libraryDependencies ++= Dependencies.basic ++ Dependencies.akka)
  ...
  lazy val mono_eth = Project("mono-eth", file("mono-eth"))
    .dependsOn(mono_base)
    .dependsOn(kesque)
    .settings(basicSettings: _*)
    .settings(noPublishing: _*)
